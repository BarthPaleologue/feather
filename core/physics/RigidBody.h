...
public:
    RigidBody(std::shared_ptr<Mesh> mesh, float mass) : PhysicsBody(mesh, mass) {
        ...
        for(unsigned int i = 0; i < mesh->vertexData().indices.size(); i+= 3) {
            ...
            addDistanceConstraint(new DistanceConstraint(p1, p2, glm::length(p1->position - p2->position), 1.0f));
            addDistanceConstraint(new DistanceConstraint(p2, p3, glm::length(p2->position - p3->position), 1.0f));
            addDistanceConstraint(new DistanceConstraint(p3, p1, glm::length(p3->position - p1->position), 1.0f));
        }
